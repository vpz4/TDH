...
def apply_final_harmonization(input_dataset_path, harmonization_report_path):
    ...
    for _, row in harmonization_df.iterrows():
        original_feature = row['Feature Name']
        matched_term = row['Matched Term']
        
        if original_feature in dataset.columns:
            rename_mapping[original_feature] = matched_term
    ...
    print(f"Harmonized dataset saved to {output_harmonized_path}")
...
def main():
    ...
    harmonization_report_path = generate_harmonization_report(matching_results, output_folder)

    # Step 6: Apply the final harmonization
    apply_harmonization = input("Do you want to apply the harmonization to an input dataset? (Y/N): ").strip().lower()
    if apply_harmonization == 'Y':
        input_dataset_path = input("Enter the path to the dataset (.csv) for harmonization: ").strip()
        apply_final_harmonization(input_dataset_path, harmonization_report_path)
